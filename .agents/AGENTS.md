...
- **Rule**: Make sure keyboard listeners in `App.tsx` always verify that no input elements are in focus before triggering global hotkeys (like `Spacebar` for play/pause, `[`/`]` for timing latching, or `n` for new cues) to prevent keypress pollution during typing.
